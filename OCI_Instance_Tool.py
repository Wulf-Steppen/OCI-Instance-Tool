...
## init Rich Console & Table & Build Color Dict ##
console = Console()
layout = Layout()
...
parser.add_argument("--style", "-s", help="changes the color scheme of the tool", default="white")
...
while val:
    with console.status(f"[bold white]Connecting to OCI to validate resources...[/bold white] ", spinner='bouncingBall', spinner_style="bold white") as status:
        ...
        if instance_display_name in instance_names:
            print(f"Duplicate display name [bold {theme}]{instance_display_name}[/bold {theme}] found in existing resources")
            sys.exit()
